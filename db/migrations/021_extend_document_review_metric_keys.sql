-- Lets the document review flow accept the six statement metrics added in
-- 020_extend_financial_metric_keys.sql.
--
-- The review flow (015, 016) keeps its own copy of the original seven metric
-- keys, so a statement with cost of sales or balance sheet lines was rejected
-- at review even though financial_metric_observations accepts them. This
-- widens both copies to the same thirteen keys:
--   1. the metric_key check on document_extraction_candidates (from 015)
--   2. the key list inside confirm_document_extraction (last defined in 016)
--
-- Widening only. The function body is 016's unchanged apart from that list,
-- so the runway-currency rules and every other check behave exactly as before.

ALTER TABLE public.document_extraction_candidates
  DROP CONSTRAINT IF EXISTS document_extraction_candidates_metric_key_check;

ALTER TABLE public.document_extraction_candidates
  ADD CONSTRAINT document_extraction_candidates_metric_key_check
  CHECK (
    metric_key IS NULL
    OR metric_key IN (
      'cash',
